...
- [I. OVERVIEW](#i-overview)
  - [1. "C# là ngôn ngữ strongly typed nghĩa là gì?"](#1-c-là-ngôn-ngữ-strongly-typed-nghĩa-là-gì)
  - [2. "Thông tin mà một kiểu dữ liệu lưu trữ gồm những gì?"](#2-thông-tin-mà-một-kiểu-dữ-liệu-lưu-trữ-gồm-những-gì)
  - [3. "Specifying types in variable declarations" – Cách khai báo kiểu cho biến](#3-specifying-types-in-variable-declarations--cách-khai-báo-kiểu-cho-biến)
    - [Kiểu tham số và kiểu trả về của phương thức](#kiểu-tham-số-và-kiểu-trả-về-của-phương-thức)
    - [Quy tắc quan trọng](#quy-tắc-quan-trọng)
  - [4. Built-in types – Các kiểu dựng sẵn trong C#](#4-built-in-types--các-kiểu-dựng-sẵn-trong-c)
  - [5. Custom types – Kiểu tự định nghĩa](#5-custom-types--kiểu-tự-định-nghĩa)
    - [Hướng dẫn chọn loại type phù hợp:](#hướng-dẫn-chọn-loại-type-phù-hợp)
  - [6. The Common Type System (CTS)](#6-the-common-type-system-cts)
    - [1) **Hỗ trợ kế thừa**](#1-hỗ-trợ-kế-thừa)
    - [2) **Mọi type đều là value type hoặc reference type**](#2-mọi-type-đều-là-value-type-hoặc-reference-type)
  - [7. Classes vs Structs vs Records](#7-classes-vs-structs-vs-records)
    - [Class](#class)
    - [Struct](#struct)
    - [Record](#record)
  - [8. Value types](#8-value-types)
    - [Hai loại:](#hai-loại)
      - [1) struct](#1-struct)
      - [2) enum](#2-enum)
  - [9. Reference types](#9-reference-types)
    - [Interface](#interface)
    - [Arrays](#arrays)
  - [10. Literal types – Kiểu của literal values](#10-literal-types--kiểu-của-literal-values)
  - [11. Generic types](#11-generic-types)
  - [12. Implicit types, anonymous types, nullable types](#12-implicit-types-anonymous-types-nullable-types)
    - [`var`](#var)
    - [Anonymous types](#anonymous-types)
    - [Nullable value types](#nullable-value-types)
  - [13. Compile-time type vs Run-time type](#13-compile-time-type-vs-run-time-type)
- [II. NAMESPACES](#ii-namespaces)
  - [1. "Namespace là gì trong C#?"](#1-namespace-là-gì-trong-c)
  - [2. "Tại sao cần namespaces?"](#2-tại-sao-cần-namespaces)
  - [3. "Cú pháp khai báo namespace"](#3-cú-pháp-khai-báo-namespace)
    - [**Dạng truyền thống (block):**](#dạng-truyền-thống-block)
    - [**Dạng file-scoped namespace (C# 10+):**](#dạng-file-scoped-namespace-c-10)
  - [4. "Nested namespaces – Namespace lồng nhau"](#4-nested-namespaces--namespace-lồng-nhau)
  - [5. "Sử dụng namespaces – từ khóa using"](#5-sử-dụng-namespaces--từ-khóa-using)
  - [6. "Global using directives (C# 10+)"](#6-global-using-directives-c-10)
  - [7. "Alias namespaces – đặt tên tắt cho namespace"](#7-alias-namespaces--đặt-tên-tắt-cho-namespace)
  - [8. "Phạm vi (scope) của namespace"](#8-phạm-vi-scope-của-namespace)
  - [9. "Namespace System là gì?"](#9-namespace-system-là-gì)
  - [10. "Namespaces không ảnh hưởng đến Value type hay Reference type"](#10-namespaces-không-ảnh-hưởng-đến-value-type-hay-reference-type)
  - [11. "Namespaces và file organization"](#11-namespaces-và-file-organization)
  - [12. "Có required namespaces không?"](#12-có-required-namespaces-không)
- [III. CLASSES](#iii-classes)
  - [1. "Class trong C# là gì?"](#1-class-trong-c-là-gì)
  - [2. "Class có những thành phần gì?"](#2-class-có-những-thành-phần-gì)
  - [3. "Cách định nghĩa một class"](#3-cách-định-nghĩa-một-class)
  - [4. "Tạo object từ class"](#4-tạo-object-từ-class)
  - [5. "Constructors – Hàm tạo"](#5-constructors--hàm-tạo)
  - [6. "Static members và static classes"](#6-static-members-và-static-classes)
    - [Static member](#static-member)
    - [Static class](#static-class)
  - [7. "Inheritance – kế thừa giữa các classes"](#7-inheritance--kế-thừa-giữa-các-classes)
  - [8. "Accessibility – các mức truy cập trong class"](#8-accessibility--các-mức-truy-cập-trong-class)
  - [9. "Object lifetime – vòng đời của object"](#9-object-lifetime--vòng-đời-của-object)
  - [10. "Partial classes"](#10-partial-classes)
  - [11. "Anonymous classes (anonymous types)"](#11-anonymous-classes-anonymous-types)
  - [12. "Classes vs Structs vs Records (summary)"](#12-classes-vs-structs-vs-records-summary)
- [IV. RECORDS](#iv-records)
  - [1. "Record là gì trong C#?"](#1-record-là-gì-trong-c)
  - [2. "Cú pháp khai báo record"](#2-cú-pháp-khai-báo-record)
    - [**Record class:**](#record-class)
    - [**Record struct:**](#record-struct)
  - [3. "Record hỗ trợ equality như thế nào?"](#3-record-hỗ-trợ-equality-như-thế-nào)
    - [Với record:](#với-record)
    - [Với class thông thường:](#với-class-thông-thường)
  - [4. "Record có immutable không?"](#4-record-có-immutable-không)
  - [5. "Vậy nondestructive mutation là gì?"](#5-vậy-nondestructive-mutation-là-gì)
  - [6. "Record có thể kế thừa không?"](#6-record-có-thể-kế-thừa-không)
    - [Record class hỗ trợ kế thừa:](#record-class-hỗ-trợ-kế-thừa)
    - [Record struct **không hỗ trợ kế thừa** (như struct).](#record-struct-không-hỗ-trợ-kế-thừa-như-struct)
  - [7. "Positional record là gì?"](#7-positional-record-là-gì)
  - [8. "Record có thể chứa methods, events và other members không?"](#8-record-có-thể-chứa-methods-events-và-other-members-không)
  - [9. "Deconstruction trong record là gì?"](#9-deconstruction-trong-record-là-gì)
  - [10. "Record struct khác gì so với record class?"](#10-record-struct-khác-gì-so-với-record-class)
  - [11. "Record phù hợp dùng khi nào?"](#11-record-phù-hợp-dùng-khi-nào)
  - [12. "Record có hỗ trợ 'primary constructor' như class không?"](#12-record-có-hỗ-trợ-primary-constructor-như-class-không)
  - [13. "Override trong record"](#13-override-trong-record)
  - [14. "Pattern matching với record"](#14-pattern-matching-với-record)
- [V. INTERFACES](#v-interfaces)
  - [1. "Interface là gì trong C#?"](#1-interface-là-gì-trong-c)
  - [2. "Khai báo interface như thế nào?"](#2-khai-báo-interface-như-thế-nào)
  - [3. "Explicit interface implementation" – triển khai tường minh](#3-explicit-interface-implementation--triển-khai-tường-minh)
  - [4. "Default interface members" – thành viên mặc định (C# 8+)](#4-default-interface-members--thành-viên-mặc-định-c-8)
  - [5. "Interface inheritance" – interface có thể kế thừa interface khác](#5-interface-inheritance--interface-có-thể-kế-thừa-interface-khác)
  - [6. "Why use interfaces?" – Tại sao cần interface?](#6-why-use-interfaces--tại-sao-cần-interface)
  - [7. "Interface vs Abstract class"](#7-interface-vs-abstract-class)
  - [8. "Checking interface implementation" – kiểm tra object có implement interface không](#8-checking-interface-implementation--kiểm-tra-object-có-implement-interface-không)
  - [9. "Interface với generic" (Generic interfaces)](#9-interface-với-generic-generic-interfaces)
  - [10. "Interface và polymorphism"](#10-interface-và-polymorphism)
- [VI. GENERICS](#vi-generics)
  - [1. "Generics trong C# là gì?"](#1-generics-trong-c-là-gì)
  - [2. "Khai báo generic class như thế nào?"](#2-khai-báo-generic-class-như-thế-nào)
  - [3. "Generic methods" – phương thức generic](#3-generic-methods--phương-thức-generic)
  - [4. "Generic interfaces"](#4-generic-interfaces)
  - [5. "Type constraints" – ràng buộc kiểu](#5-type-constraints--ràng-buộc-kiểu)
    - [Các loại constraints:](#các-loại-constraints)
      - [✔ `where T : struct`](#-where-t--struct)
      - [✔ `where T : class`](#-where-t--class)
      - [✔ `where T : new()`](#-where-t--new)
      - [✔ `where T : BaseClass`](#-where-t--baseclass)
      - [✔ `where T : interface`](#-where-t--interface)
      - [✔ Nhiều constraints cùng lúc](#-nhiều-constraints-cùng-lúc)
  - [6. "Covariance và Contravariance" (C# 4+)](#6-covariance-và-contravariance-c-4)
    - [✔ Covariance (`out`)](#-covariance-out)
    - [✔ Contravariance (`in`)](#-contravariance-in)
    - [✔ Invariance](#-invariance)
  - [7. "Benefits of generics" – Tại sao dùng generics?](#7-benefits-of-generics--tại-sao-dùng-generics)
    - [✔ 1. Type safety](#-1-type-safety)
    - [✔ 2. Hiệu năng tốt hơn](#-2-hiệu-năng-tốt-hơn)
    - [✔ 3. Tái sử dụng code](#-3-tái-sử-dụng-code)
  - [8. "Built-in generic types" – các kiểu dựng sẵn](#8-built-in-generic-types--các-kiểu-dựng-sẵn)
  - [9. "Generic delegates"](#9-generic-delegates)
  - [10. "Type inference" – suy luận kiểu](#10-type-inference--suy-luận-kiểu)
- [VII. TUPLES \& ANONYMOUS TYPES](#vii-tuples--anonymous-types)
- [PART A — ANONYMOUS TYPES](#part-a--anonymous-types)
  - [1. "Anonymous type là gì?"](#1-anonymous-type-là-gì)
  - [2. "Cú pháp khởi tạo anonymous type"](#2-cú-pháp-khởi-tạo-anonymous-type)
  - [3. "Anonymous types trong LINQ"](#3-anonymous-types-trong-linq)
  - [4. "Anonymous types là reference type hay value type?"](#4-anonymous-types-là-reference-type-hay-value-type)
  - [5. "Equality của anonymous type"](#5-equality-của-anonymous-type)
  - [6. "Anonymous types và scope"](#6-anonymous-types-và-scope)
- [PART B — TUPLES (Bổ sung để hoàn chỉnh Section VI)](#part-b--tuples-bổ-sung-để-hoàn-chỉnh-section-vi)
  - [7. "Tuple là gì?"](#7-tuple-là-gì)
  - [8. "Tuple deconstruction"](#8-tuple-deconstruction)
  - [9. "Difference: Tuple vs Anonymous Type"](#9-difference-tuple-vs-anonymous-type)
  - [10. "Khi nào dùng tuple, khi nào dùng anonymous type?"](#10-khi-nào-dùng-tuple-khi-nào-dùng-anonymous-type)
    - [Dùng **anonymous type** khi:](#dùng-anonymous-type-khi)
    - [Dùng **tuple** khi:](#dùng-tuple-khi)
